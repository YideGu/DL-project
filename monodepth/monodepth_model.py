...
class MonodepthModel(object):
    ...
    def get_disparity_smoothness(self, disp, pyramid): 
        disp_gradients_x = [self.gradient_x(d) for d in disp]       # gradient |dx{d^l_ij}| term
        disp_gradients_y = [self.gradient_y(d) for d in disp]       # gradient |dy{d^l_ij}| term
    ...
    def build_outputs(self):
        ...
        with tf.variable_scope('disparities'):
            ...
            self.disp_left_est  = [tf.expand_dims(d[:,:,:,0], 3) for d in self.disp_est]            # d^l   (from right to left)
            self.disp_right_est = [tf.expand_dims(d[:,:,:,1], 3) for d in self.disp_est]            # d^r   (from left to right)
        ...
        with tf.variable_scope('images'):
            self.left_est  = [self.generate_image_left(self.right_pyramid[i], self.disp_left_est[i])  for i in range(4)]            # I_l_bar = I^r(d^l)
            self.right_est = [self.generate_image_right(self.left_pyramid[i], self.disp_right_est[i]) for i in range(4)]            # I_r_bar = I^l(d^r)

        # LR CONSISTENCY
        with tf.variable_scope('left-right'):
            self.right_to_left_disp = [self.generate_image_left(self.disp_right_est[i], self.disp_left_est[i])  for i in range(4)]  # d^r_{ij+d^l_ij}
            self.left_to_right_disp = [self.generate_image_right(self.disp_left_est[i], self.disp_right_est[i]) for i in range(4)]

        # DISPARITY SMOOTHNESS  # terms in C_ds^l and C_ds^r under different scale
        with tf.variable_scope('smoothness'):
            self.disp_left_smoothness  = self.get_disparity_smoothness(self.disp_left_est,  self.left_pyramid)          # C_ds^l
    ...
    def build_losses(self):
        with tf.variable_scope('losses', reuse=self.reuse_variables):
            ...
            # L1
            self.l1_left = [tf.abs( self.left_est[i] - self.left_pyramid[i]) for i in range(4)]     # |I_bar_ij^l - I_ij^l|
            self.l1_reconstruction_loss_left  = [tf.reduce_mean(l) for l in self.l1_left]
            self.l1_right = [tf.abs(self.right_est[i] - self.right_pyramid[i]) for i in range(4)]   # |I_bar_ij^r - I_ij^r|
            self.l1_reconstruction_loss_right = [tf.reduce_mean(l) for l in self.l1_right]

            # SSIM
            self.ssim_left = [self.SSIM( self.left_est[i],  self.left_pyramid[i]) for i in range(4)]    # (1-SSIM(I_bar_ij^l, I_ij^l))/2
            self.ssim_loss_left  = [tf.reduce_mean(s) for s in self.ssim_left]
            self.ssim_right = [self.SSIM(self.right_est[i], self.right_pyramid[i]) for i in range(4)]   # (1-SSIM(I_bar_ij^r, I_ij^r))/2
            self.ssim_loss_right = [tf.reduce_mean(s) for s in self.ssim_right]

            # WEIGTHED SUM
            # C_ap^r for each scale
            ...
            # DISPARITY SMOOTHNESS  # C_ds^l and C_ds^r under different scale
            self.disp_left_loss  = [tf.reduce_mean(tf.abs(self.disp_left_smoothness[i]))  / 2 ** i for i in range(4)]               # C_ds^l for each scale
            self.disp_right_loss = [tf.reduce_mean(tf.abs(self.disp_right_smoothness[i])) / 2 ** i for i in range(4)]
            self.disp_gradient_loss = tf.add_n(self.disp_left_loss + self.disp_right_loss)

            # LR CONSISTENCY
            self.lr_left_loss  = [tf.reduce_mean(tf.abs(self.right_to_left_disp[i] - self.disp_left_est[i]))  for i in range(4)]    # C_lr^l for each scale
            self.lr_right_loss = [tf.reduce_mean(tf.abs(self.left_to_right_disp[i] - self.disp_right_est[i])) for i in range(4)]    # C_lr^r for each scale
